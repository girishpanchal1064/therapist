                ...
                @forelse($blocks as $i => $block)
                  <tr>
                    ...
                    <td class="d-flex gap-1">
                      ...
                      <form method="POST" action="{{ route('therapist.availability.block.toggle', $block) }}">
                        @csrf
                        <button class="btn btn-sm btn-warning" type="submit">
                          <i class="ri-refresh-line me-1"></i> {{ $block->is_active ? 'Restore' : 'Disable' }}
                        </button>
                      </form>
                      <form method="POST" action="{{ route('therapist.availability.block.destroy', $block) }}" onsubmit="return confirm('Delete this block?')">
                        @csrf
                        @method('DELETE')
